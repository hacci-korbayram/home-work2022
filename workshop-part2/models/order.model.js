...
const ordersPath = path.join(__dirname, "..", "data", "order.json");
...
class OrderModel {
    ...
    static async addNewOrder(newOrderData) {
        ...
        const nameExists = orders.some(order => order.name === newOrderData.name);
    ...
    static async patchOrder(orderId, orderUpdateData){
        const orders = await this.getAllOrderById();
