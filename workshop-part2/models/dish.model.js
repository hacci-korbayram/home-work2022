...
class DishModel {
    ...
    static async addNewDish(newDishData) {
        ...
        if (nameExists) return Promise.reject({ msg: "resgistered" });
        ...
        if (newDish.price <= 1 || newDish.price >= 1000) {
            return Promise.reject({ msg: `there can be no price less than 1 and more than 1000` });
    ...
    static async patchDish(dishId, dishUpdateData) {
        ...
        const foundDish = await this.getAllDishById(dishId);

        const updateDish = { ...foundDish, dishUpdateData };

        const updateDishs = dishs.map(dish =>
            dish.id === foundDish.id ? updateDish : dish
        );
    ...
    static async deleteDish(dishId){
        ...
        const updateDish = dishs.filter(dish => dish.id === dishId);

        if(updateDish.length === dishs.length)
        
        return Promise.reject({msg: "dish not found"});

        await DataService.deleteJSONFile(dishsPath, updateDish);
    }
}
